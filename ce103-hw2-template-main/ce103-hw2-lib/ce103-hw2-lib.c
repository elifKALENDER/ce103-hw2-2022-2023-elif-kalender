...
int ce103_strcmp(const char* fiLhs, const char* fiRhs)
{
	...
	int ret;
	//Compares two null-terminated byte strings lexicographically // filhs ve firhsnin karakter olarak büyüklüğüne bakıyor. eğer eşitse döngüden çıkıyor eğer değilse filhsyi bir artırıyor.
...
void ce103_hex2bin(char* fiHex, int fiHexLen, unsigned char* foBin)
{
	...
	int remainder, binaryval=0, i=0, hexadecimalval = 0;
...
int ce103_gcd(int fiNum1, int fiNum2)
{
	//TODO:Start from Here...
	int i, gcd;
		//we continue untill "i" gets the value of one of them
		for (i = 1;i <= fiNum1 && i <= fiNum2;++i)
		{
			//we continue untill we find their biggest value, without any reminder
